...
Bike::Bike(float startX, float startY, float w, float h, float moveSpeed)
...
Bike::Bike(float startX, float startY, float w, float h, float moveSpeed, SDL_Color color)
	:Bike(startX, startY, w, h, moveSpeed)
{
	this->color = color;
}

Bike::Bike(float startX, float startY, float w, float h, float moveSpeed, SDL_Color color, PlayState* world)
	:Bike(startX, startY, w, h, moveSpeed)
{
	this->world = world;
}
...
void Bike::update(float deltaTime)
{
	if (isAlive) {
		...
		float newX = getPosition().x + (direction.x * moveSpeed * deltaTime);
		float newY = getPosition().y + (direction.y * moveSpeed * deltaTime);
		translate(newX, newY);
